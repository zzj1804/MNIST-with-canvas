...
ui.setCleanCanvasButtonCallback(() => {
    ui.setCanvasResult('');
});

ui.setDrawCanvasCallback(() => {
    if (!model) return;
    let imgData = ui.getCanvasImageData();
    let data = new Float32Array(IMAGE_H * IMAGE_W);

    for (let i = 0; i < imgData.length / 4; i++) {
        data[i] = imgData[i * 4] / 255;
    }

    let xs = tf.tensor4d(data, [1, IMAGE_H, IMAGE_W, 1]);
    let output = model.predict(xs);
    let prediction = output.dataSync();
    let predictNum = output.argMax(1).dataSync();

    let res = [];
    for (let i = 0; i < prediction.length; i++) {
        res.push({num: i, pred: prediction[i]});
    }
    res.sort((a,b) => b.pred - a.pred);

    let message = '';
    for (let i = 0; i < 3; i++) {
        message += ` ${res[i].num}: ${(res[i].pred * 100).toFixed(2)}%`;
    }
    ui.setCanvasResult(message);
});
